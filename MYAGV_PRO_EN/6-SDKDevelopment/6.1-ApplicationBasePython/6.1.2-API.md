...
#### get_pin_input(pin): {#get_pin_inputpin}
...
#### set_pin_output(pin, state): {#set_pin_outputpin-state}
...
agv_pro = MyAGVPro("/dev/agvpro_controller", baudrate=1000000, debug=True)
...
#### 6.3 Using Socket Communication
- Obtaining the AGVPro Wi-Fi Account and Password in Serial Communication Mode
  ```python
  from pymycobot import MyAGVPro
  
  # 1. Initialize the MyAGVPro object
  agv_pro = MyAGVPro("/dev/agvpro_controller", baudrate=1000000, debug=True)
  
  # 2. Obtain the AGVPro Wi-Fi account and password
  account, password = agv_pro.get_wifi_account()
  
  # 3. Print the account and password
  print(f"Account: {account}, Password: {password}")
  # Output: Account: ElephantAGVPro-AP2.4G, Password: elephant
  
  # 4. Obtain the configured IP address and open port for the AGVPro
  ip, port = agv_pro.get_wifi_ip_port()
  
  # 5. Print IP address and open port
...
- WIFI connection
  ...
  Connect to AGVPro WIFI using the obtained WIFI account and password.

- Initializing Socket Communication
  ```python
  from pymycobot import MyAGVProSocket
  
  # 1. Initialize the MyAGVProSocket object based on the obtained IP address and port number.
  agv_pro_socket = MyAGVProSocket("192.168.4.1", 9000, debug=True)
  
  # 2. Get the AGVPro version information.
